...
- simpl. reflexivity.
- simpl in IHx.
induction y; simpl; try reflexivity.
inversion H. f_equal.
...
Lemma rev1:
forall n : nat,
rev (repeat I n) = repeat I n.
Proof.
induction n; simpl; try reflexivity.
rewrite IHn. rewrite rep1, rep2. reflexivity.
Qed.

Lemma empty (X : Type) (a : list X):
a ++ [] = a.
Proof.
...
Lemma assoc (X : Type) (a b c : list X):
(a ++ b) ++ c = a ++ (b ++ c).
Proof.
induction a; simpl; try reflexivity.
rewrite IHa. reflexivity.
...
Lemma rev2 (X : Type) (a b : list X):
rev (a ++ b) = rev b ++ rev a.
Proof.
induction a; simpl; try reflexivity.
- rewrite empty. reflexivity.
- rewrite IHa. rewrite assoc. reflexivity.
Qed.

Lemma sth:
forall n : nat,
AddLr (repeat I n) (repeat I n) I = repeat I n.
Proof.
induction n; simpl; try reflexivity.
rewrite IHn. reflexivity.
Qed.
...
Lemma nott:
forall n,
NotL (repeat I n ++ [O]) = repeat O n ++ [I].
Proof.
induction n; simpl; try reflexivity.
rewrite IHn. reflexivity.
Qed.
...
rewrite H, H0. rewrite not.
unfold AddL.
rewrite rev1.
induction n; simpl. destruct H1. reflexivity.
rewrite sth.
rewrite rev1.
rewrite nott.
reflexivity.
